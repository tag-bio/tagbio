...
#'
#' A tagResult is typically pre-populated and passed to a user-
#' defined R function.  The type and paths direct the user what output
#' is expected and where results should be saved.
#'
#' @param result_data a data frame of results
#' @param output_path path to results file
...
#' Add data to a tagResult object
#'
...
#' @export
add_result <- function(tag_result, results, result_type) {
  UseMethod("add_result", tag_data, results, result_type)
}

#' @export
add_result.tagData <- function(tag_result, results, result_type) {
  if (
    !(result_type %in% c("data.frame", "html", "jpeg", "pdf", "png", "json"))
  ) {
