#' Tag.bio templates from R Markdown
#'
#' This function is a slight reworking of the
#' \code{\link[prettydoc]{html_pretty}} method used to create HTML documents
#' from R Markdown that work well within the Tag.bio platform.
#'
#' @param theme Character string to specify the document theme.
#' Currently supported themes are \code{"cayman"}, \code{"tactile"},
#' \code{"architect"}, \code{"leonids"}, and \code{"hpstr"}.
#' @param highlight Character string to specify the syntax highlight theme.
#' Supported values are \code{NULL} (use the Pandoc default), \code{kate},
#' \code{monochrome}, \code{breezeDark}, \code{espresso}, \code{zenburn},
#' \code{haddock}, and \code{tango}.
...
#' @author J Ireland, Yixuan Qiu (prettydoc)
#' @examples
#' \dontrun{
#' doc <- system.file("rmarkdown", "templates", "tagbio",
...
tag_report <- function(
  ...
  # use a different CSS from rmarkdown::html_document(), which can be done by
  # passing the `css` parameter to Pandoc. rmarkdown::html_vignette() is a good
  # and simple example of designing new output format, and in fact html_pretty()
  # is largely based on rmarkdown::html_vignette() with some other features
  # added in.
  #
  ...
  css_dir <- system.file("resources", "css", package = "tagbio")
  hl_dir <- system.file("resources", "css", "highlight", package = "tagbio")
  font_dir <- system.file("resources", "fonts", package = "tagbio")
  img_dir <- system.file("resources", "images", package = "tagbio")
  js_dir <- system.file("resources", "js", package = "tagbio")
  tmpl_dir <- system.file("resources", "templates", package = "tagbio")
  ...
  if (!isTRUE(theme %in% avail_themes)) {
    warning("theme not found, use default (default-color) instead")
    theme <- "default-color"
  }
  theme_css <- file.path(
    css_dir,
  ...
  ## if logo supplied, copy it over
  base_logo <- NULL
  if (!is.null(logo)) {
    base <- basename(logo)
    base_logo <- paste("images", base, sep = "/")
    final_logo <- file.path(img_dir, base)
    file.copy(logo, final_logo)
  }

  ## Borrowed from rmarkdown::html_vignette()
  ...
  pre_processor <- function(
    ...
    if (math == "katex") {
      ...
      pandoc_flags <- c(
        pandoc_flags,
        "--variable",
        sprintf("resource-dir=%s", files_dir),
        "--variable",
        "katex"
      )
    } else {
  ...
  ## rmarkdown::html_document() disables certain features (including MathJax)
  ## when we are using a template file other than default. I guess this is to
  ...
  # add in logo if given
  if (!is.null(base_logo)) {
    pandoc_args <- append(pandoc_args, "--metadata")
    pandoc_args <- append(pandoc_args, paste("logo", base_logo, sep = ":"))
    res$pandoc$args <- pandoc_args
  }
