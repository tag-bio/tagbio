...
#' \code{tagFC} objects are not normally created directly, but are instead
#' created using the \code{tbl} method, much like how a database connection
#' is made through \code{dbplyr}.
#'
#' @section Methods:
#' \code{summary(tc)}: Shows all collections in this data product.  Results are
#' returned in a table with columns.
#'
#' @section Miscellaneous:
#' Data products at tag.bio were historically called "flux capacitors"
#' or "FC"s for short.  The "fc" name lives on in some of the internal
#' classes and methods.
...
#' Useful links:
#' \itemize{
#' \item \url{https://backtothefuture.fandom.com/wiki/Flux_capacitor}
#' for the origin of "flux capacitor"
#' }
#'
#'
#' @slot con a tagConnect object
#' @slot fc name of the data product.
#' When running locally, this should be left empty
#' @slot qdelim delimiter character used to separate collection names
#' and collection values
#' @slot qselect list of attribute selections.  Normally this should be set up
#' using \code{select} methods
#' @slot qfilter list of item filters.  Normally this should be set up
#' using \code{filter} methods
#' @slot collection_defs definitions of available collections for this data
#' product. Populated when a tagbio_fc object is first created.
...
tag_summary_row <- function(x) {
  res <- switch(
    ...
    `numeric-matrix` = tibble::tibble(
      collection = x$collection,
      collection_type = x$data_function_type,
      collection_size = x$collection_size,
      collection_entity_count = x$collection_entity_count
    ),
    `categorical-matrix` = tibble::tibble(
      collection = x$collection,
      collection_type = x$data_function_type,
      collection_size = x$collection_size,
      collection_entity_count = x$collection_entity_count
    ),
    `data-frame-numeric` = tibble::tibble(
      collection = x$collection,
      collection_type = x$data_function_type,
      collection_size = x$collection_size,
      collection_entity_count = x$collection_entity_count
  ...
  for (av in analysis_variables) {
    if (av$data_function_type == "numeric") {
      if (is.null(av$variable)) {
        # not a variable
        num_colls[[cnt]] <- list(
          "data_function_type" = av$data_function_type,
...
get_collection_defs.tagFC <- function(.data) {
  ...
  if (length(.data$collection_defs) == 0) {
    ...
    jsonPayload[["script"]] <- script
    jsonPayload[["stringify_names"]] <- "true"
    # collections_json <- fc_post_call("q",
    #                                  .data$url,
    #                                  .data$con$api_key,
    #                                  "json",
    #                                  jsonPayload,
    #                                  token = .data$con$token)
...
get_variable_defs.tagFC <- function(.data, ...) {
  ...
  anl_vars <- lapply(collection_list, function(b) {
    list(
      "data_function_type" = collection_defs[[b]]$data_function_type,
...
get_background.tagFC <- function(.data) {
  ...
  criteria_list <- list()
  for (i in c(seq_along(.data$qfilter))) {
    # TODO - check that results are allowable types...
...
parse_collection_values <- function(res_values) {
  ...
  tag_coll <- switch(
    variable_type,
    numeric = NumericCollection(
      collection = res_values$collection,
      collection_size = res_values$`collection-size`
    ),
    categorical = CategoricalCollection(
      collection = res_values$collection,
      collection_size = res_values$`collection-size`,
      collection_entity_count = res_values$`collection-entity-count`
    ),
    `numeric-matrix` = CategoricalCollection(
      collection = res_values$collection,
      collection_size = res_values$`collection-size`,
      collection_entity_count = res_values$`collection-entity-count`
    ),
    `categorical-matrix` = CategoricalCollection(
      collection = res_values$collection,
      collection_size = res_values$`collection-size`,
      collection_entity_count = res_values$`collection-entity-count`
    ),
    `data-frame-numeric` = CategoricalCollection(
      collection = res_values$collection,
      collection_size = res_values$`collection-size`,
      collection_entity_count = res_values$`collection-entity-count`
    )
...
parse_collection_query <- function(query_res) {
  ...
  collection_defs <- lapply(res, function(x) {
    parse_collection_values(x$values)
  })
  collection_defs <- purrr::set_names(
    collection_defs,
    lapply(
      collection_defs,
      function(x) {
...
parse_variable_query <- function(query_res) {
  ...
  res <- query_res$results
  variable_defs <- lapply(res, function(x) {
    parse_variable_values(x$values)
  })
  variable_defs <- purrr::set_names(
    variable_defs,
    lapply(variable_defs, function(x) {
      x$variable
    })
  )
...
get_info <- function(url, tc) {
  ...
  # update time stamps
  info_json$start_time <- as.POSIXct(
    as.numeric(info_json$start_time) / 1000,
    origin = "1970-01-01",
    tz = "GMT"
  )
  info_json$data_timestamp <- as.POSIXct(
    as.numeric(info_json$data_timestamp) / 1000,
    origin = "1970-01-01",
    tz = "GMT"
  )
