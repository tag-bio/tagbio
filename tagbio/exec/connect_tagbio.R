...
suppressWarnings(suppressPackageStartupMessages(library("tagbio")))
...
rmd_reader <- function(rmd_file) {
  ...
  list(yaml_content = yaml_content, other_content = other_content)
}
...
rmd_updater <- function(rmd_file, email, analysis_url) {
  ...
  yaml <- yaml::read_yaml(text = paste(rmd$yaml_content, collapse = "\n"))
  ...
  if (!("analysis_url" %in% yaml_fields) || is.na(yaml["analysis_url"])) {
    if (!is.null(analysis_url)) {
      # stops the auto formatting
      analysis_url <- gsub("https://", "", analysis_url)
    }
...
if (!(args$output_type %in% c("csv", "html", "json", "pdf", "png", "tar"))) {
  stop(
    "output_type must be one of: csv, html, json, pdf, png, tar.
       See script usage (--help)"
  )
}

## Read in the fc params and create FC and protocol instances
fc_data <- rjson::fromJSON(file = args$fc_data)
...
if (
  !is.null(tag_params) &&
    !is.null(tag_params$output_file) &&
    !is.null(tag_params$output_type)
) {
  tag_result <- add_result(
    tag_result,
    tag_params$output_file[1],
    tag_params$output_type[1]
  )
  output_set <- TRUE
}
